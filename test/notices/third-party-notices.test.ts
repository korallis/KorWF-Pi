...
// @ts-expect-error — plain ESM script without a .d.ts; the shapes are asserted below.
import { checkNotices, parseHeader, HEADER_RE } from "../../scripts/check-third-party-notices.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const repoRoot = join(here, "../..");
const HEADER = "Adapted from pi 0.86.1 examples/extensions/dirty-repo-guard.ts — MIT, © Mario Zechner / earendil-works";
...
describe("THIRD_PARTY_NOTICES.md (repository)", () => {
  ...
  it("every file citing a Pi example in its header is listed in THIRD_PARTY_NOTICES.md", () => {
    ...
    expect([...result.listed.keys()].sort()).toEqual([...result.attributed.keys()].sort());
  });
});
...
function fixture(rows: string[], files: Record<string, string>): string {
  ...
  writeFileSync(
    ...
    readFileSync(join(repoRoot, "docs/adr/0001-reuse-of-pi-examples.md"), "utf8"),
  );
  const table = [
...
describe("attribution check (fixtures)", () => {
  ...
  it("FAILS when a file cites a Pi example but is missing from the notices", () => {
    ...
    try {
      ...
      expect(errors[0]).toMatch(/src\/git\/status\.ts cites examples\/extensions\/dirty-repo-guard\.ts .* not listed/);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("FAILS when a listed file has lost its header (stale notice)", () => {
    ...
    try {
      expect(checkNotices(root).errors[0]).toMatch(/carries no ADR 0001 attribution header/);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("FAILS when the listed Pi version disagrees with the header", () => {
    const root = fixture(["| `src/git/status.ts` | `dirty-repo-guard.ts` | 0.86.0 | 7 |"], {
      "src/git/status.ts": attributedFile,
    });
    try {
      expect(checkNotices(root).errors[0]).toMatch(/notices list pi 0\.86\.0 but header cites pi 0\.86\.1/);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("FAILS when the header cites an example outside the ADR 0001 copy manifest", () => {
    ...
    const root = fixture(["| `src/git/status.ts` | `todo.ts` | 0.86.1 | 4 |"], { "src/git/status.ts": bad });
    try {
      expect(checkNotices(root).errors[0]).toMatch(/not in the ADR 0001 copy manifest/);
    } finally {
      rmSync(root, { recursive: true, force: true });
    }
  });
  ...
  it("recognises exactly the ADR 0001 header format", () => {
    expect(parseHeader(`// ${HEADER}\n`)).toEqual({ version: "0.86.1", source: "dirty-repo-guard.ts" });
    expect(parseHeader("// Adapted from pi examples, MIT\n")).toBeUndefined();
    expect(HEADER_RE.test("Adapted from pi 0.86.0 examples/extensions/subagent/agents.ts — MIT, © Mario Zechner / earendil-works")).toBe(true);
  });
});
