...
| Attempt | worker id → `workerId`; task profile → `taskProfile`; requested model → `requestedModel`; used model → `usedModel`; fallback reason → `fallbackReason`; profile → `profile`; inputs → `inputs`; worktree → `worktree`; timestamps → `timestamps`; usage → `usage`; outcome → `outcome`; artifacts → `artifacts`. Extra: `taskRevision`, `role`, `handedOffFromAttemptId`, `termination` (#124). |
